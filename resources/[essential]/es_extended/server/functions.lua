...
ESX.SavePlayer = function(xPlayer, cb)
	...
	for i=1, #xPlayer.accounts, 1 do
		if ESX.LastPlayerData[xPlayer.source].accounts[xPlayer.accounts[i].name] ~= xPlayer.accounts[i].money then
			table.insert(asyncTasks, function(cb)
				exports.ghmattimysql:execute('UPDATE user_accounts SET money = @money WHERE identifier = @identifier AND name = @name', {
	...
	for i=1, #xPlayer.inventory, 1 do
		if ESX.LastPlayerData[xPlayer.source].items[xPlayer.inventory[i].name] ~= xPlayer.inventory[i].count then
			table.insert(asyncTasks, function(cb)
				exports.ghmattimysql:execute('UPDATE user_inventory SET count = @count WHERE identifier = @identifier AND item = @item', {
	...
	table.insert(asyncTasks, function(cb)
		exports.ghmattimysql:execute('UPDATE users SET job = @job, job_grade = @job_grade, loadout = @loadout, position = @position WHERE identifier = @identifier', {
