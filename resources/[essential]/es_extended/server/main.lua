					...
					if not found then
						...
						local scope = function(item, identifier)
							table.insert(tasks2, function(cb2)
								exports.ghmattimysql:scalar('INSERT INTO user_inventory (identifier, item, count) VALUES (@identifier, @item, @count)', {
		...
		table.insert(tasks, function(cb)
			...
			table.insert(tasks2, function(cb2)
				...
				exports.ghmattimysql:execute('SELECT * FROM users WHERE identifier = @identifier', {
					...
					for i = 1, #result do 
						local job, grade = result[i].job, tostring(result[i].job_grade)
						...
						if result[i].loadout ~= nil then
							userData.loadout = json.decode(result[i].loadout)
						...
						if result[i].position ~= nil then
							userData.lastPosition = json.decode(result[i].position)
						end
