...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
	...
	if xPlayer.job.name == 'cardealer' and xTarget then
		exports.ghmattimysql:scalar('SELECT id FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			['@vehicle'] = model
		}, function(result)
			if result[1] then
...
ESX.RegisterServerCallback('esx_vehicleshop:getSoldVehicles', function(source, cb)
	exports.ghmattimysql:scalar('SELECT client, model, plate, soldby, date FROM vehicle_sold', {}, function(result)
		cb(result)
	end)
end)
...
AddEventHandler('esx_vehicleshop:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	...
	if xPlayer.job.name == 'cardealer' and xTarget then
		exports.ghmattimysql:scalar('SELECT id, price FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			['@vehicle'] = vehicle
		}, function(result)
			if result[1] then
				...
				exports.ghmattimysql:execute('DELETE FROM cardealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						exports.ghmattimysql:execute('INSERT INTO rented_vehicles (vehicle, plate, player_name, base_price, rent_price, owner) VALUES (@vehicle, @plate, @player_name, @base_price, @rent_price, @owner)', {
							['@vehicle']     = vehicle,
							['@plate']       = plate,
...
ESX.RegisterServerCallback('esx_vehicleshop:getCommercialVehicles', function(source, cb)
	exports.ghmattimysql:scalar('SELECT price, vehicle FROM cardealer_vehicles ORDER BY vehicle ASC', {}, function(result)
		cb(result)
	end)
end)
...
ESX.RegisterServerCallback('esx_vehicleshop:getRentedVehicles', function(source, cb)
	exports.ghmattimysql:execute('SELECT * FROM rented_vehicles ORDER BY player_name ASC', {}, function(result)
		local vehicles = {}
...
ESX.RegisterServerCallback('esx_vehicleshop:giveBackVehicle', function(source, cb, plate)
	exports.ghmattimysql:scalar('SELECT base_price, vehicle FROM rented_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
		if result[1] then
			...
			exports.ghmattimysql:execute('DELETE FROM rented_vehicles WHERE plate = @plate', {
				['@plate'] = plate
			}, function(rowsChanged)
				exports.ghmattimysql:execute('INSERT INTO cardealer_vehicles (vehicle, price) VALUES (@vehicle, @price)', {
					['@vehicle'] = vehicle,
...
ESX.RegisterServerCallback('esx_vehicleshop:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'cardealer' then
		...
		if not resellPrice then
			...
			exports.ghmattimysql:execute('SELECT * FROM rented_vehicles WHERE plate = @plate', {
				['@plate'] = plate
			}, function(result)
...
ESX.RegisterServerCallback('esx_vehicleshop:isPlateTaken', function(source, cb, plate)
	exports.ghmattimysql:scalar('SELECT 1 FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
		cb(result ~= nil)
	end)
...
function PayRent(d, h, m)
	...
	exports.ghmattimysql:scalar('SELECT owner, rent_price FROM rented_vehicles', {}, function(result)
		for k, v in ipairs(result) do 
			table.insert(tasks, function(cb)
				local xPlayer = ESX.GetPlayerFromIdentifier(result.owner)

				if xPlayer then -- message player if connected
					xPlayer.removeAccountMoney('bank', result.rent_price)
					xPlayer.showNotification(_U('paid_rental', ESX.Math.GroupDigits(result.rent_price)))
				else -- pay rent by updating SQL
					MySQL.Sync.execute('UPDATE users SET bank = bank - @bank WHERE identifier = @identifier', {
						['@bank'] = result.rent_price,
						['@identifier'] = result.owner
					})
				end

				TriggerEvent('esx_addonaccount:getSharedAccount', 'society_cardealer', function(account)
					account.addMoney(result.rent_price)
				end)
