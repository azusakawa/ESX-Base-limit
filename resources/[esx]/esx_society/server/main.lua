...
Citizen.CreateThread(function()
	exports.ghmattimysql:execute('SELECT * FROM jobs', {}, function(jobs)
		for i = 1, #jobs do
			...
			Jobs[jobs[i].name].grades = {}
		end
	end)

	exports.ghmattimysql:execute('SELECT * FROM job_grades', {}, function(grades)
		for i = 1, #grades do
			Jobs[grades[i].job_name].grades[tostring(grades[i].grade)] = grades[i]
		end
	end)
end)
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	if Config.EnableESXIdentity then
		...
		exports.ghmattimysql:scalar('SELECT firstname, lastname, identifier, job, job_grade FROM users WHERE job = @job ORDER BY job_grade DESC', {
			['@job'] = society
		}, function (results)
			local employees = {}
		...
		exports.ghmattimysql:execute('SELECT * FROM users WHERE job = @job ORDER BY job_grade DESC', {
			['@job'] = society
		}, function (result)
			local employees = {}
