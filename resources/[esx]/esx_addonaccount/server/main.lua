...
Citizen.CreateThread(function()
	exports.ghmattimysql:execute('SELECT * FROM addon_account', {}, function(result)
		for i=1, #result, 1 do
			local name   = result[i].name
			local label  = result[i].label
			local shared = result[i].shared

			exports.ghmattimysql:execute('SELECT * FROM addon_account_data WHERE account_name = @account_name', {['@account_name'] = name}, function(result2)
				if shared == 0 then
