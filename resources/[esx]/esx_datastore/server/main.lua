...
Citizen.CreateThread(function()
	exports.ghmattimysql:execute('SELECT * FROM datastore', {}, function(result)
		for i=1, #result do
			local name   = result[i].name
			local label  = result[i].label
			local shared = result[i].shared

			exports.ghmattimysql:execute('SELECT * FROM datastore_data WHERE name = @name', {['@name'] = name}, function(result2) 
				if shared == 0 then
					...
					if #result2 == 0 then
						exports.ghmattimysql:execute('INSERT INTO datastore_data (name, owner, data) VALUES (@name, NULL, \'{}\')', {
							['@name'] = name
						})
