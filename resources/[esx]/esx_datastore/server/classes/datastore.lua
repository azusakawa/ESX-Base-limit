	...
	self.save = function()
		...
		local timeoutCallback = ESX.SetTimeout(10000, function()
			if self.owner == nil then
				exports.ghmattimysql:execute('UPDATE datastore_data SET data = @data WHERE name = @name', {
					['@data'] = json.encode(self.data),
					['@name'] = self.name,
				})
			else
				exports.ghmattimysql:execute('UPDATE datastore_data SET data = @data WHERE name = @name and owner = @owner', {
					['@data']  = json.encode(self.data),
					['@name']  = self.name,
					['@owner'] = self.owner,
				})
