...
ESX.RegisterServerCallback('esx_policejob:getOtherPlayerData', function(source, cb, target)
	if Config.EnableESXIdentity then
		...
		local result = exports.ghmattimysql:scalar('SELECT firstname, lastname, sex, dateofbirth, height FROM users WHERE identifier = @identifier', {
...
ESX.RegisterServerCallback('esx_policejob:getVehicleInfos', function(source, cb, plate)
	...
	exports.ghmattimysql:scalar('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		...
		if result[1] then
			exports.ghmattimysql:scalar('SELECT name, firstname, lastname FROM users WHERE identifier = @identifier',  {
...
ESX.RegisterServerCallback('esx_policejob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	...
	for k,v in ipairs(nearbyVehicles) do
		local result = exports.ghmattimysql:scalar('SELECT plate FROM owned_vehicles WHERE owner = @owner AND plate = @plate AND job = @job', {
			['@owner'] = xPlayer.identifier,
			['@plate'] = v.plate,
			['@job'] = xPlayer.job.name
		})
